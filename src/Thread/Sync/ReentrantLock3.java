...
public class ReentrantLock3 implements BankAccount {

    private double balance;
    private final Lock lock = new java.util.concurrent.locks.ReentrantLock();

    public ReentrantLock3(int balance) {
    ...
    public boolean withdraw(double amount) {
        ...
        try {
            if(!lock.tryLock(500, TimeUnit.MICROSECONDS)) {
                System.out.println( Thread.currentThread().getName() +"[진입 실패] 이미 처리중인 작업이있음");
                return false;
            }
